...
def _scene_specs():
    ...
    for i in range(32):
        ...
        out.append(dict(number=i+1,stage=step["id"],title=f"{step['civil']} Â· {aspect}",aspect=aspect,step=step))
    return out
...
def render_stage_gallery(stage_id):
    ...
    st.caption(f"{len(scenes)} distinct scenes on this page Â· 32 across the complete companion app")
    tabs=st.tabs([f"{s['number']:02} Â· {s['aspect']}" for s in scenes])
    for tab,scene in zip(tabs,scenes):
        with tab:
            st.markdown(f"#### Illustration {scene['number']:02} Â· {scene['title']}")
