...
const root=path.dirname(fileURLToPath(import.meta.url)),src=fs.readFileSync(path.join(root,"build_nb.py"),"utf8"),pyCore=fs.readFileSync(path.join(root,"guardian.py"),"utf8");
...
blocks.__preamble__=pyCore.slice(0,pyCore.search(/^def\s+/m)).trim();
...
const nb={cells,metadata:{kernelspec:{display_name:"Python 3",language:"python",name:"python3"},language_info:{name:"python",version:"3"},colab:{name:"Guardian_Road_AI_Safety_Shield.ipynb",provenance:[]}},nbformat:4,nbformat_minor:5};
const out=path.join(root,"Guardian_Road_AI_Safety_Shield.ipynb");fs.writeFileSync(out,JSON.stringify(nb,null,1)+"\n","utf8");console.log(`Wrote ${path.basename(out)}: ${cells.length} cells`);
