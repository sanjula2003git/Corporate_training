...
def mat(name,color,rough=.35):
    ...
    return m
wall=mat('Epicardium',(.58,.235,.26),.4)
rim=mat('Cut myocardium',(.52,.055,.085),.52)
blue=mat('Venous vessel tissue',(.37,.29,.40),.33)
red=mat('Arterial vessel tissue',(.62,.35,.35),.34)
gold=mat('Ivory valve leaflets',(.78,.64,.53),.38)
...
def organic(id,t,a,r,inside):
    ...
    if inside:relief+=.018*math.sin(t*22+a*5)*math.sin(a)**2
    else:relief+=.003*math.sin(t*88+a*12)
    x*=1+relief;y*=1+relief
    return (x,y,zz)

def bowl(id,c,r,thickness):
    parent=group(id,c);verts=[];faces=[];n=64;m=64
...
for id,c,r,thick in [('right_ventricle',(-.48,0,-.43),(.67,.65,1.02),.095),('left_ventricle',(.49,.08,-.51),(.72,.7,1.18),.20)]:
    ...
    for j in range(17):
        a=.25+j*2.65/17;ts=[.70+h*.1 for h in range(17)]
        points=[tuple(Vector(c)+Vector(organic(id,t,a+.11*math.sin(t*4+j),ri,True))) for t in ts]
        o=tube(id,points,.014+(j%3)*.003,wall,False,'Trabeculae');o['internal_detail']=True
...
# Small-scale muscle fibers use an embedded normal map, supported by the web viewer.
size=256;image=bpy.data.images.new('Muscle_microfibers',width=size,height=size);pix=[]
for y in range(size):
    for x in range(size):
        u=x/size;v=y/size
        dx=.16*math.sin(2*math.pi*(u*37+v*6))+.07*math.sin(2*math.pi*(u*79-v*11))
        dy=.08*math.cos(2*math.pi*(v*61+u*3))
        norm=Vector((dx,dy,1)).normalized();pix.extend((norm.x*.5+.5,norm.y*.5+.5,norm.z*.5+.5,1))
image.colorspace_settings.name='Non-Color';image.pixels=pix;image.pack()
for material in [wall,rim,inner,red,blue]:
...
bpy.ops.export_scene.gltf(filepath=str(ROOT/'viewer'/'heart.glb'),export_format='GLB',export_extras=True,export_animations=True,export_morph=True,export_apply=False,export_vertex_color='ACTIVE')
for o in bpy.data.objects:
...
# Also render the whole-heart surface for review.
for o in bpy.data.objects:
    if o.get('front_cover') or o.get('exterior_detail'):o.hide_render=False;o.hide_set(False)
    q=o;internal=False
    while q:
        internal=internal or q.get('internal_detail') or q.get('part_id')=='septum';q=q.parent
    if internal:o.hide_render=True;o.hide_set(True)
scene.render.filepath=str(ROOT/'assets'/'heart-exterior.png');bpy.ops.render.render(write_still=True)
print('Detailed heart GLB, Blender source and both previews created.')
