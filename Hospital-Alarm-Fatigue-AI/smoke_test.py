...
def main():
    ...
    edges = [
        ("start", {"Alerts allowed per hour": 1}),
        ("start", {"Alerts allowed per hour": 15}),
        ("budget", {"Nurses on shift": 1}),
        ("budget", {"Nurses on shift": 4}),
        ("manager", {"Alerts allowed per hour": 1}),
        # A budget of 1 is the setting where the manager catches no event at all,
        # which is the branch that shows the warning instead of the patient trace.
        ("patient", {"Alerts allowed per hour": 1}),
        ("patient", {"Alerts allowed per hour": 15}),
        ("scoreboard", {"Sensor glitches per patient per day": 4}),
        ("scoreboard", {"Sensor glitches per patient per day": 40}),
    ]
