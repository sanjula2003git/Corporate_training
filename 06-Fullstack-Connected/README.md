# Stage 6 — Full-Stack Connected + Role-Based Portals (React ↔ FastAPI ↔ SQLite)

**ABC Learning Solutions — Training Management Portal**

> This is the **same React UI as `03-React-Version`**, but the mock data layer
> has been swapped for **real API calls** to the backend (`05-Backend-Database`).
> Every screen now reads and writes the **live SQLite database** — completing
> the full-stack loop: **Frontend → API → Database**.
>
> `03-React-Version` is kept untouched as the *pure-frontend* teaching example.
> This folder is its *connected* twin, so you can show both side by side.
