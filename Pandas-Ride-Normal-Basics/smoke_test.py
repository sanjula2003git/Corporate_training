"""Walk every page of the app and not allowed loudly on the first exception.
...
def main():
    ...
    edges = [
        ("start", dict(Visitors=20)),
        ("start", dict(Visitors=200)),
        ("dupes", {"Duplicated rows glued": 0}),
        ("holes", {"Empty cells": 0}),
        ("holes", {"Empty cells": 20}),
        ("fill", {"Fill the numeric holes": "mean"}),
        ("fill", {"Fill the numeric holes": "median"}),
        # No "Extreme ... planted" dial here: this project studies clean normal data,
        # so story.EXTREMES is empty and the app never builds that slider.
        ("payoff", {"Visitors": 20, "Empty cells": 20}),
