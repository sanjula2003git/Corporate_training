...
def render_load():
    st.title("⑤ Materials arrive on site")
    st.info("🧱 **Like unloading a delivery.** The lorry drops the materials at the gate. "
            "You haven't checked them yet — you've just got them on site. That's *loading data*.")
...
def render_inspect():
    st.title("⑥ Check the cement bags")
    st.info("🧱 **You would never pour cement without checking the bags.** Torn ones, damp ones, "
            "wrong grade. You check *before* you build, not after. Same with data.")
...
def render_clean():
    st.title("⑦ Remove the damaged bricks")
    st.info("🧱 **You throw the cracked bricks off the pallet before you build.** A cracked brick in a "
            "wall is a weak wall. A broken sensor reading in your data is a broken model — and unlike "
            "the wall, you won't be able to *see* it.")
...
def render_normalize():
    st.title("⑧ Standardize the materials")
    st.info("🧱 **Every sensor uses a different unit — gas in ppm, noise in decibels, distance in metres.** "
    ...
    n1.metric("Gas reads", "420 ppm")
    n2.metric("Noise reads", "85 dB")
    n3.metric("Distance reads", "3 m")
    st.caption("Same site, same second. To a model, gas looks **140× more important than distance** — "
               "purely because of its units. That's not engineering, that's an accident. Press Play:")
    d = get_data()
...
def render_split():
    st.title("⑨ Practice work vs the final inspection")
    st.info("🧱 **Never test someone on the exact work they practised on.** "
            "They would just repeat what they memorised, and you would learn nothing about their real "
            "skill. So we keep some shifts aside that the model never trains on, and score it only on those.")
...
def render_what_is_dl():
    ...
    st.markdown("#### Why depth = recognizing deeper patterns")
    a, b = st.columns(2)
    a.markdown(
    ...
    b.markdown(
        ...
        "Same principle, different data. The network learns these patterns from examples.")
...
def render_activation():
    ...
    st.info({"Sigmoid": "0–1 output → final layer of a yes/no model (our risk output).",
             "Tanh": "−1…1, zero-centerd → older hidden layers.",
...
def render_network():
    ...
    st.caption("Change the architecture, then press ▶ Play to see how each layer builds on the last.")
    d = get_data()
    ...
    with st.expander("🔬 Go deeper — the exact numbers going in"):
    ...
    # ---- 2. INSIDE one neuron ----
    st.markdown("#### 2 · Inside ONE neuron — the arithmetic, step by step")
    st.caption("It is just multiply, add, then bend. Press ▶ Play to watch "
    ...
    st.info("**That is learning.** Each weight moves a *tiny* fraction against its own gradient. "
            ...
            "No insight, no intuition — just arithmetic, millions of times.")
    with st.expander(f"🔬 Go deeper — the actual weight updates (layer 1, neuron {jn+1})"):
        st.caption("gradient × learning rate. This table *is* the learning.")
        st.dataframe(pd.DataFrame({
            "Weight (from)": NICE,
    ...
    exp = st.expander("🔬 Go deeper — the per-neuron arithmetic (z, bias, activation)")
    tabs = exp.tabs([f"Hidden layer {i+1}" for i in range(n_layers)] + ["Output"])
...
def render_cnn_input():
    ...
    st.info("**Where it fails.** `helmet_03` and `helmet_04` (helmets *worn on heads*) come back "
            ...
            "the guess is wrong. This is the current limit of the model, not a bug in our demo.")


def render_feature_maps():
    st.title("㉔ Inside the CNN — how the image becomes a helmet decision")
    st.markdown("#### A helmet is hard to write as a rule. Here is what the network learned instead.")
    imgs, _ = cnn_meta()
    if not imgs:
        _no_assets(); return
    ...
    st.markdown(
        ...
        "gap:14px 18px;text-align:center;padding:12px 0'>"
        + "<span style='opacity:.55;font-size:18px'>→</span>".join(
            f"<span style='color:{MUTED};font-size:14px;white-space:nowrap'>{s}</span>"
            for s in ["🧍 Worker", "📐 Edges", "🪖 Helmet shape",
                      "🧠 Helmet features", "✅ Helmet detected"])
        + "</div>", unsafe_allow_html=True)
    st.caption("Drag the slider to move through these steps on a real photo. The network learns each "
               "step from examples, rather than being told the rule.")
    ...
    st.caption("Each tile is one real learned filter. Edges → parts → abstraction as depth grows, and "
               "the network learns all of it from examples.")
    ...
    conv_exp = st.expander("🔬 Go deeper — what a 'filter' actually is (just 27 numbers)")
    W, bb = conv1_kernels()
    if W is None:
        conv_exp.warning("conv1_kernels.npz missing — rerun tools/precompute_cnn.py.")
    else:
      with conv_exp:
    ...
    myths = [
        ("“The CNN sees the photo the way you do.”",
         "It receives a grid of numbers — 224×224×3 = 150,528 of them, normalized. No objects, no scene, "
         "no worker. Just numbers. Everything else is arithmetic on that grid."),
        ("“A filter is a complicated AI thing.”",
         "A first-layer filter is **9 numbers per color channel** — 27 in total. You just watched all 27 get "
        ...
        ("“Each filter detects an object.”",
         "Early filters fire on edges and color blobs — they'd fire the same on a helmet, a cup, or a road "
        ...
        ("“Grad-CAM shows what the network was thinking.”",
         ...
         "**why**."),
        ("“It must see the whole helmet to recognize one.”",
...
def render_gradcam():
    ...
    st.markdown("#### Reading Grad-CAM carefully")
    for m, r in [
        ("“Grad-CAM shows what the network was thinking.”",
...
def render_compare_proof():
    st.title("The proof — each tool doing the part it is good for")
    st.table(pd.DataFrame({
        "": ["Sensor risk (table)", "Helmet from raw pixels", "Who names the features?"],
        "ML — Random Forest": ["✅ works", "❌ can't even start", "The engineer"],
        "DL — ANN / CNN": ["✅ works", "✅ edges → parts → helmet", "The network learns them"],
    }))
    st.success("On the sensor table both tools work, because the engineer already named the factors. "
               "On raw pixels — where no one can hand-write the rules — DL takes that part off the "
               "engineer's plate. AI does not out-think the human here; it just handles the job a person "
               "cannot do by hand, so the two together cover the whole site.")
...
# ============================================================================
# THE MANPOWER DASHBOARD — the closing page
#
# Kept deliberately simple: four numbers and one chart. Every hour here is
# arithmetic on the three inputs above it, not a measurement — this app has
# never seen a real site's timesheet. The task minutes below are the course's
# standing assumptions for the site; change them here, in one place.
# ============================================================================
# minutes, per shift, for the safety work on a site of this size
MANUAL_MIN = [("Patrolling the zones", lambda crew, zones: 6 * zones * 8),
              ("Writing the shift log", lambda crew, zones: 6 * 5),
              ("Checking PPE by hand", lambda crew, zones: 2 * crew * 1.5),
              ("Shift + audit report", lambda crew, zones: 40)]
AI_MIN = [("Walking out to alerts", lambda crew, zones: 6 * 10),
          ("Checking what the system wrote", lambda crew, zones: 18),
          ("Keeping the system alive", lambda crew, zones: 12)]


def _tile(col, label, value, sub, color):
    col.markdown(
        f"<div style='background:{PANEL};border-radius:12px;padding:14px 16px;height:124px;"
        f"border-top:3px solid {color}'>"
        f"<div style='color:{MUTED};font-size:11px;letter-spacing:.6px'>{label}</div>"
        f"<div style='color:{color};font-size:30px;font-weight:700;line-height:1.5'>{value}</div>"
        f"<div style='color:{MUTED};font-size:12px;line-height:1.4'>{sub}</div></div>",
        unsafe_allow_html=True)


def render_manpower():
    st.title("What the system gave back")
    st.caption("The safety work on this site, in man-hours — by hand, and with the system. "
               "Move the sliders to your own site.")

    c1, c2, c3 = st.columns(3)
    crew = c1.slider("Workers on site", 4, 60, 18, 1)
    shift_h = c2.slider("Shift length (hours)", 4.0, 12.0, 9.0, 0.5)
    days = c3.slider("Working days per month", 20, 31, 26, 1)
    zones = 4  # tunnel, yard, two upper decks — the site from Phase 1

    manual = [(n, f(crew, zones) / 60) for n, f in MANUAL_MIN]
    ai = [(n, f(crew, zones) / 60) for n, f in AI_MIN]
    manual_h = sum(v for _, v in manual)
    ai_h = sum(v for _, v in ai)
    saved = manual_h - ai_h
    pct = saved / manual_h * 100 if manual_h else 0
    fte = saved / shift_h if shift_h else 0

    k = st.columns(4)
    _tile(k[0], "BY HAND", f"{manual_h:.1f} h",
          f"of every {shift_h:g} h shift", AMBER)
    _tile(k[1], "WITH THE SYSTEM", f"{ai_h:.1f} h", "the same job, per shift", POS)
    _tile(k[2], "HOURS GIVEN BACK", f"{saved * days:,.0f} h",
          f"per month · {pct:.0f}% less work", GREEN)
    _tile(k[3], "THAT IS THIS MANY PEOPLE", f"{fte:.2f} FTE",
          "freed up for the judgement work only a person can do", GREEN)

    st.write("")
    fig = go.Figure()
    for name, val in manual:
        fig.add_trace(go.Bar(y=["By hand"], x=[val], name=name, orientation="h",
                             marker_color=AMBER, text=f"{name} · {val:.1f}h",
                             textposition="inside", insidetextanchor="middle",
                             marker_line=dict(color=BG, width=2), showlegend=False,
                             hovertemplate=f"{name}: %{{x:.2f}} h<extra></extra>"))
    for name, val in ai:
        fig.add_trace(go.Bar(y=["With the system"], x=[val], name=name, orientation="h",
                             marker_color=POS, text=f"{name} · {val:.1f}h",
                             textposition="inside", insidetextanchor="middle",
                             marker_line=dict(color=BG, width=2), showlegend=False,
                             hovertemplate=f"{name}: %{{x:.2f}} h<extra></extra>"))
    fig.update_layout(barmode="stack", xaxis_title="man-hours in one shift",
                      title="One shift's safety workload")
    st.plotly_chart(style(fig, 300), use_container_width=True, key="mp_bars")

    st.info(f"**{saved:.1f} hours a shift, {saved * days:,.0f} a month, {fte:.2f} of a person.** "
            f"The bottom bar is not empty and never should be — automation moves the work, it does "
            f"not delete it. And nobody is sacked: this site was short of eyes, not long on staff, "
            f"so those hours go back into toolbox talks and the crew having a bad day.")
    st.caption("These are the course's assumptions for this site, not measurements from a real one. "
               "Put your own project's timesheet numbers in and the arithmetic still holds.")
...
# ---- the course, as ONE CONSTRUCTION DAY -----------------------------------
# Every renderer below is the *technical explanation* (Part 4) of its page.
# bridge.open_page() puts the Civil Engineering context, the engineering
# challenge and the AI connection ABOVE it; bridge.close_page() puts the
# notebook connection BELOW it. The renderers themselves are untouched.
STAGES = {
    # the project overview: the problem, the goal, the mind map, the mapping
    "start":           ("⓪ The project — read this first",
                        lambda: bridge.render_start(style, animate)),
    # ACT 1 — why AI exists at all
    "site":            ("① A Tuesday on site", lambda: story.render_site(style, animate)),
    "enter-ai":        ("② Enter AI", lambda: story.render_enter_ai(style, animate)),
    "inspection":      ("③ Morning inspection", lambda: story.render_inspection(get_data, style, animate)),
    "two-worlds":      ("④ Two worlds", lambda: story.render_two_worlds(style)),
    # ACT 2 — the materials
    "load":            ("⑤ Materials arrive", render_load),
    "inspect":         ("⑥ Check the bags", render_inspect),
    "clean":           ("⑦ Remove bad bricks", render_clean),
    "normalize":       ("⑧ Standardize", render_normalize),
    "split":           ("⑨ Practice vs audit", render_split),
    "ml-baseline":     ("⑩ ML on the numbers", render_ml_baseline),
    # ACT 3 — the wall that makes DL inevitable
    "pixel-problem":   ("⑪ The pixel problem", lambda: story.render_pixel_problem(cnn_meta, _asset, style)),
    "handmade-features": ("⑫ Build it by hand", lambda: story.render_handmade(cnn_meta, _asset, style)),
    "why-dl":          ("⑬ Therefore: Deep Learning", lambda: story.render_why_dl(style)),
    # ACT 4 — how a machine learns
    "supervisor-brain": ("⑭ A supervisor's brain", lambda: story.render_supervisor_brain(style)),
    "neuron":          ("⑮ The neuron", render_neuron),
    "activation":      ("⑯ Activation", render_activation),
    "learning-loop":   ("⑰ Mistake → adjust → repeat", lambda: story.render_learning_loop(style, animate)),
    "loss":            ("⑱ Loss function", render_loss),
    "gradient-descent": ("⑲ Gradient descent", render_gradient_descent),
    "learning-rate":   ("⑳ Learning rate", render_learning_rate),
    "optimizer":       ("㉑ Optimizers", render_optimizer),
    "network":         ("㉒ The network", render_network),
    "training":        ("㉓ Training", render_training),
    # ACT 5 — vision
    "cnn-journey":     ("㉔ Inside the CNN", render_feature_maps),
    "gradcam":         ("㉕ Grad-CAM proof", render_gradcam),
    # ACT 6 — the product
    "audit":           ("㉖ The site audit", lambda: story.render_audit(get_data, get_models, style, animate)),
    "proof":           ("㉗ The proof", render_compare_proof),
    ...
    # ACT 7 — the business case
    "manpower":        ("㉚ The manpower ledger", render_manpower),
}

# old links keep working
ALIASES = {
    "overview": "site", "compare": "two-worlds", "what-is-dl": "network",
    "cnn-input": "pixel-problem", "feature-maps": "cnn-journey",
    "evaluate": "audit", "compare-proof": "proof", "fusion": "fusion-engine",
}
...
with st.sidebar:
    st.markdown("### 🦺 A Man Management Dilemma")
    st.caption("You are running a construction project, and AI keeps turning out to be the thing "
               "that eases the load one person cannot carry alone.")
    keys = list(STAGES)
    sel = st.selectbox("Where are we on the job?", keys, index=keys.index(stage),
                       format_func=lambda k: STAGES[k][0])
    if sel != stage:
        st.query_params["stage"] = sel
        st.rerun()

    # the sidebar tells you the ENGINEERING phase, not the lecture number
    if stage in bridge.BY_ID:
        ...
        st.markdown(
            ...
            f"<span style='color:#8b949e'>SITE ACTIVITY</span><br>"
    ...
    st.caption("🔊 Every step has narration at the bottom.")

# ---- the five-part page -----------------------------------------------------
# Parts 1-3 (site context -> challenge -> AI connection), then the EXISTING
# renderer as Part 4, then Part 5 (notebook connection) and the step navigation.
if stage != "start":
